...
The Celery application is defined in `backend/celery.py`. Unlike other components, Celery does NOT read configuration from the project's Settings class. Instead, Celery must be configured using environment variables.
...
```python
from backend.services.db import get_engine, AsyncSession
...
celery.conf.beat_schedule = {
    ...
    'send-weekly-report': {
        'task': 'backend.tasks.send_weekly_report',
        'schedule': crontab(day_of_week='monday', hour=9, minute=0),
    },
    'check-status-every-5-min': {
        'task': 'backend.tasks.check_status',
...
from backend.tasks import email, reports, cleanup
```
...
celery -A backend.celery worker --loglevel=info --concurrency=4

# With specific queues
celery -A backend.celery worker --loglevel=info -Q celery,emails
```
...
```bash
# List active tasks
celery -A backend.celery inspect active

# List scheduled tasks (ETA tasks)
celery -A backend.celery inspect scheduled

# List registered tasks
celery -A backend.celery inspect registered

# Worker statistics
celery -A backend.celery inspect stats

# Ping workers
celery -A backend.celery inspect ping
```
...
from backend.celery import celery, hello_world
...
def test_periodic_tasks_registered():
    ...
    from backend.celery import setup_periodic_tasks
...
def test_cache_setup_handler_exists():
    ...
    from backend.celery import setup_caches
...
def test_task_with_database(monkeypatch):
    ...
    with patch('backend.services.db.get_session', return_value=mock_session):
...
export CELERY_BROKER_URL="redis://localhost:6379/0"
celery -A backend.celery worker --loglevel=debug --pool=solo
```
