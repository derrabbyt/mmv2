...
pytest --cov=./backend/services/cache.py tests/services/test_cache.py
```
...
```python
@pytest.mark.asyncio
@patch('backend.services.cache.get_cached')
...
def test_celery_task_registration():
    ...
    from backend.celery import celery, my_task

    assert "backend.celery.my_task" in celery.tasks

def test_celery_task_execution():
    """Test direct task execution."""
    from backend.celery import my_task
...
def test_settings_load():
    ...
    assert settings.project_name == "backend"
