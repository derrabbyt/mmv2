...
The FastAPI application is defined in `backend/www.py` and includes:

- **Automatic API documentation** at `/docs` (Swagger UI) and `/redoc` (ReDoc)
- **Static file serving** from `backend/static/` via the `/static/` endpoint
...
Create a new route in `backend/www.py`:
...
uvicorn backend.www:app --reload --host 0.0.0.0 --port 8000
...
uvicorn backend.www:app --host 0.0.0.0 --port 8000 --workers 4

# Or using gunicorn with uvicorn workers
gunicorn backend.www:app -w 4 -k uvicorn.workers.UvicornWorker
```
