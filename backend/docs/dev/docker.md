...
ENV APP_MODULE=backend.www:app
...
ENV APP_MODULE=backend.celery:celery
...
docker build -f dockerfile.www -t backend-www:latest .

# Tag for registry
docker tag backend-www:latest ghcr.io/your-org/backend-www:latest

# Push to registry
docker push ghcr.io/your-org/backend-www:latest
```
...
services:
  www:
    image: ghcr.io/your-org/backend-www:latest
    restart: always
...
# Run a command
docker-compose exec www python -m backend.cli version

# Check database connection
docker-compose exec www python -c "from backend.services.db import engine; print(engine)"
```
